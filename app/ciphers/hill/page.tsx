...
export default function HillCipher() {
  ...
  const encryptMessage = () => {
    ...
    // Prepare plaintext
    const numbers = prepareText(plaintext)

    const cipher = []
    const steps: string[] = []
    steps.push(`Starting with plaintext: ${plaintext}`)
    steps.push(`Converted to numbers: ${numbers.join(", ")}`)

    // Process in blocks
    const blockSize = Number.parseInt(matrixSize)
    for (let i = 0; i < numbers.length; i += blockSize) {
      const block = numbers.slice(i, i + blockSize)
      const encryptedBlock = multiplyMatrix(keyMatrix, block)

      steps.push(`Block ${i / blockSize + 1}: [${block.join(", ")}] → [${encryptedBlock.join(", ")}]`)
      cipher.push(...encryptedBlock)
    }

    // Convert back to text
    const cipherText = numbersToText(cipher)
  ...
  const decryptMessage = () => {
    ...
    // Prepare ciphertext
    const numbers = prepareText(ciphertext)
    ...
    for (let i = 0; i < numbers.length; i += blockSize) {
      const block = numbers.slice(i, i + blockSize)
      const decryptedBlock = multiplyMatrix(inverseMatrix, block)

      steps.push(`Block ${i / blockSize + 1}: [${block.join(", ")}] → [${decryptedBlock.join(", ")}]`)
      plain.push(...decryptedBlock)
    }

    // Convert back to text
    const plainText = numbersToText(plain)
