...
export default function ExtendedEuclid() {
  ...
  const calculateInverse = () => {
    ...
    if (inverse < 0) inverse += numM // Ensure positive result
  ...
  const extendedGCD = (a: number, b: number) => {
    ...
    steps.push(`Initial values: r₀ = ${old_r}, r₁ = ${r}, s₀ = ${old_s}, s₁ = ${s}, t₀ = ${old_t}, t₁ = ${t}`)

    let iteration = 1
    while (r !== 0) {
      const quotient = Math.floor(old_r / r)
      steps.push(`Iteration ${iteration}: ${old_r} = ${quotient} × ${r} + ${old_r - quotient * r}`)
      ...
      steps.push(`  Updated values: r₍${iteration}₎ = ${r}, s₍${iteration}₎ = ${s}, t₍${iteration}₎ = ${t}`)
    ...
    if (old_r === 1) {
      steps.push(`Bézout coefficients: s = ${old_s}, t = ${old_t}`)
      steps.push(`Therefore, ${a} × ${old_s} + ${b} × ${old_t} = 1`)
      steps.push(`The modular inverse of ${a} modulo ${b} is ${old_s < 0 ? old_s + b : old_s}`)
    } else {
      steps.push(`Since GCD ≠ 1, the modular inverse does not exist`)
    }
